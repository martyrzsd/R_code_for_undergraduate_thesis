...
# non sparse
{
  ...
  final_plot_rho0 <- c()
  {
    ...
    final_plot_rho0 <-
      as.data.frame(matrix(final_plot_rho0, nrow = 4, byrow = TRUE))
...
# Sparse
{
  ...
  sparseB <- matrix(rbinom(p * input_dimension, 2, 0.1), nrow = p)
  B <- sparseB * B
  ...
  for (correction_methods in c("none", "BBC", "RMT")) {
    ...
    # Name the out put by the correction methods used
    assign(paste0("sparse ", correction_methods),
           results_for_one_method)
    sparse_final_plot_rho09[length(sparse_final_plot_rho09) + 1] <-
      sum(get(correction_methods))
  }
}
